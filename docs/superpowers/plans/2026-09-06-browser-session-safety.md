...
- The implementation owner writes only its designated isolated checkout. Other checkouts and any shared dependency symlink targets are read-only. No concurrent source owners or worker-spawned agents. Round4 ownership was handed to the fresh Terra checkout `/Users/mac/Downloads/Awoof/.worktrees/browser-harness-fix4` from exact b008716; the earlier native7783 checkout stays frozen. See the plan ledger and fix-round4 brief for the bounded continuation.
...
`app/layout.tsx` currently imports next/font/google: browser route blocking does not govern build-time Node fetches. The round4 parent ruling, announced to the owner, permits the unchanged font's public build assets from fonts.googleapis.com/fonts.gstatic.com as a normal build dependency. This narrowly supersedes the initial offline-only gate; it does not establish offline reproducibility or permit real business API/provider calls. Use no copied real environment or credentials, keep business APIs synthetic loopback, and report font-fetch failure as a build blocker. Do not change production typography or add an environment-dependent production font workaround in this task.
