...
## Lifecycle fixture correction validation (2026-09-05)

- [x] The startup-timeout fixture now records an explicit `startup-entered` marker, remains alive only in that branch, and is given a 1-second startup allowance. The lifecycle assertion requires the marker, `error.code === 'ETIMEDOUT'`, and an absent child PID before cleanup.
- [x] Cleanup tests use unique local subprocess fixtures and actual owned scratch/data directories. Failed-stop, clean exact-status-3 confirmation, uncertain post-stop, setup-failure, and timed-start cleanup assert the on-disk removal or retention result; every lifecycle status/stop child receives the exact `-D` data directory and an asserted stop child.
- [x] RED: the prior fallthrough fixture returned normally under the realistic timeout (`startup.error.code` was `undefined`, not `ETIMEDOUT`). GREEN: `node --test scripts/test-postgres.test.mjs` passed 4/4 on three consecutive runs; `npm run test:postgres` passed 18/18 against the guarded disposable database; `npm run type-check` and `git diff --check` passed. No lifecycle-helper change was needed because the corrected executable fixtures confirmed its existing policy.
