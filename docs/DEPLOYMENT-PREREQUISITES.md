...
Migration 036 enforces one widget configuration per vendor. It fails atomically if historical duplicates exist; reconcile their keys/domains deliberately before retrying. Config creation and updates now use an atomic upsert. The migration also restores locally expired initialized checkouts to pending: a local 24-hour timer cannot invalidate a Paystack authorization. Initialized or uncertain reservations remain blocked/reusable until the provider outcome is conclusively reconciled.

Payout updates lock the current user and vendor through the bounded provider mutation and local apply, serializing suspension/deletion with payout changes. Confirmed rejection or lost authority before a provider call releases the pending guard; an indeterminate provider/local-commit outcome remains pending reconciliation. Browser CORS preflights no longer consume the user action quota. Enrollment controls require successful school email confirmation in the current renewal flow.


Migration 037 adds an eligibility-specific payment reconciliation reason. Payment completion acquires current student authority before transaction/product/vendor locks and rechecks the evidence deadline before stock is consumed. A successful charge without current eligibility is queued for refund reconciliation. The old public university verification-method configuration route now returns 410; clients use the sanitized availability route. Verification status exposes mailbox proof for the current account email, allowing enrollment recovery without another OTP when that proof already exists.
