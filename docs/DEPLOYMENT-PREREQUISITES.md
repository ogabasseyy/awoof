...
Existing students can renew verification at `/student/verification` using the current processing notice, school email challenge, and enrollment check. Logout accepts the captured refresh credential and conditionally revokes only its matching stored session. Bank-account resolution now uses POST `/api/vendors/payment/resolve-account` with a JSON body.
