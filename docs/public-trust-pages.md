...
- `/privacy` and `/terms`: existing broken footer links need valid destinations, but substantive legal text needs confirmed entity/contact details, actual data flows/retention and owner/legal approval. Do not publish generic boilerplate as an approved policy. Record this as a release dependency; do not substitute an empty placeholder policy or remove required signup notices silently. Interim mapping: the shared footer exposes Privacy and Terms under a Legal column pointing at `/trust`, whose Limits section records both documents as pending owner approval. Replace with standalone routes only when approved content exists.
