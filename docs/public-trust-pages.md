...
- `/privacy` and `/terms`: existing broken footer links need valid destinations, but substantive legal text needs confirmed entity/contact details, actual data flows/retention and owner/legal approval. Do not publish generic boilerplate as an approved policy. Record this as a release dependency; do not substitute an empty placeholder policy or remove required signup notices silently. The shared footer deliberately carries no Privacy/Terms labels until owner-approved routes exist — pointing them at `/trust` would misrepresent the trust center as their replacement. Add the labels with standalone routes only when approved content exists.
...
| Support happens in-app after sign-in; no public inbox or phone is offered | `/contact` | Support routes `/student/profile/support`, `/vendor/support` exist; `support@awoof.tech` monitoring unverified, `+2348000000000` is a placeholder — both omitted | Pending — deploy/canary/partner gates NOT RUN (cutover runbook) | Needs support owner before any inbox/phone is added | 2026-09-21 | Implemented design; publish pending deployment evidence |
