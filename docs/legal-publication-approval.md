...
- Deployment ordering: `docker-compose.hostinger.yml` gates web startup on the backend image's health check; backend startup is already gated on successful migration. Keep that dependency chain intact and do not bypass it with `--no-deps`: the new signup bundle must not reach users while the old strict API is still active. This is a source-level rollout safeguard, not evidence of successful production deployment. No additional public policy promises are introduced by this ordering or the test-runner timeout adjustment.
