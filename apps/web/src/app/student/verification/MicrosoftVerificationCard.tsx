...
import { isMicrosoftAuthorizationUrl, tabAttemptExpiresAt, writeMicrosoftAttempt } from '@/lib/microsoft-verification';
...
export default function MicrosoftVerificationCard({ available, unavailableReason, parentAccepted, setParentAccepted, processingGrant, onMicrosoftIdentityUnlinked }: Props) {
    ...
    const [eligibilityRefreshMessage, setEligibilityRefreshMessage] = useState('');
    ...
    const loadHistory = async (cursor?: string) => {
        ...
        try {
            ...
            setNextCursor(response.data.data.nextCursor); setHistoryLoaded(true); setHistoryError(''); return true;
        } catch {
    ...
    const start = () => void run(async () => {
        ...
        // without comparing clocks from different hosts.
        writeMicrosoftAttempt(window.sessionStorage, { attemptId: started.data.data.attemptId, finishSecret: started.data.data.finishSecret, browserSessionId, expiresAt: tabAttemptExpiresAt(started.data.data.expiresAt, started.data.data.serverNow) });
    ...
    const unlink = (identity: Identity) => void run(async () => {
        ...
        if (eligibilityRefresh.status === 'rejected') { setEligibilityRefreshMessage('The connection was removed, but current eligibility could not be refreshed.'); setEligibilityRefreshError(true); }
    });
    const withdrawProviderConsent = (consent: History) => void run(async () => {
        await microsoftVerificationApiClient.post(`/verification/microsoft/consents/${consent.id}/withdraw`, {});
        if (!current()) return;
        ++historyRead.current;
        setHistory(previous => previous.map(item => item.id === consent.id ? { ...item, withdrawnAt: item.withdrawnAt ?? new Date().toISOString() } : item));
        setMessage('Microsoft provider consent withdrawn. Independent email evidence was not changed.');
        const [historyRefresh, eligibilityRefresh] = await Promise.allSettled([loadHistory(), onMicrosoftIdentityUnlinked()]);
        if (!current()) return;
        if (historyRefresh.status !== 'fulfilled' || !historyRefresh.value) setHistoryError('Microsoft consent was withdrawn, but consent history could not be refreshed. Use Refresh to check it again.');
        if (eligibilityRefresh.status === 'rejected') { setEligibilityRefreshMessage('Microsoft consent was withdrawn, but current eligibility could not be refreshed.'); setEligibilityRefreshError(true); }
    });
    const retryEligibility = () => void (async () => {
        if (!current()) return;
        setBusy(true);
        try { await onMicrosoftIdentityUnlinked(); if (current()) { setEligibilityRefreshError(false); setEligibilityRefreshMessage(''); } }
