...
test('later consent pages do not duplicate a locally granted consent', async ({ page }) => {
    ...
    await page.route(`${apiOrigin}/api/verification/**`, async (route) => {
        ...
        let data: unknown;
        if (url.pathname.endsWith('/microsoft/identities')) {
            expect(route.request().method()).toBe('GET'); data = { items: [], nextCursor: null };
        } else if (url.pathname.endsWith('/status')) data = { email: 'student@approved.test', emailDomainApproved: true, universityId: 'school', eligibility: { eligible: false }, notices: { verification: { version: 'v1', text: 'Consent notice' } } };
...
for (const heldRead of ['status', 'methods'] as const) {
    test(`withdrawal supersedes an older ${heldRead} response in the same session`, async ({ page }) => {
        ...
        await page.route(`${apiOrigin}/api/verification/**`, async (route) => {
            ...
            let data: unknown;
            if (path.endsWith('/microsoft/identities')) {
                expect(route.request().method()).toBe('GET'); data = { items: [], nextCursor: null };
...
test(`an older consent history response preserves new and existing grants (history=${hasHistory})`, async ({ page }) => {
    ...
    await page.route(`${apiOrigin}/api/verification/**`, async (route) => {
        ...
        if (path.endsWith('/microsoft/identities')) {
            expect(route.request().method()).toBe('GET'); data = { items: [], nextCursor: null };
        } else if (path.endsWith('/status')) data = { email: 'student@approved.test', emailDomainApproved: true, universityId: 'school', eligibility: { eligible: false }, notices: { verification: { version: 'v1', text: 'Consent notice' } } };
    ...
    try {
        await page.goto('/student/verification'); await page.getByRole('checkbox', { name: 'Consent notice' }).check();
        await page.getByRole('button', { name: 'Send verification code' }).click();
...
test('withdrawing a new grant reloads history and ignores the superseded initial page', async ({ page }) => {
    ...
    let release!: () => void;
    const held = new Promise<void>((resolve) => { release = resolve; });
    let heldInitialReads = 0;
    let deliveredInitialReads = 0;
    let withdrawn = false;
    ...
    await page.route(`${apiOrigin}/api/verification/**`, async (route) => {
        ...
        if (path.endsWith('/microsoft/identities')) {
            ...
            if (!withdrawn) {
                ...
                heldInitialReads += 1;
                await held;
                headers['x-awoof-fixture-initial-consent-read'] = 'true';
                data = { items: [], nextCursor: null };
            } else data = { items: [entry('old-grant'), entry('new-grant', '2026-09-11T00:00:00Z')], nextCursor: 'new-grant' };
        } else if (path.endsWith('/consents/new-grant')) {
            expect(route.request().method()).toBe('DELETE');
            withdrawn = true;
            data = { grantId: 'new-grant' };
        }
        else if (path.endsWith('/initiate')) data = { processingGrantId: 'new-grant' };
        else data = { challengeId: 'challenge', resendAvailableAt: new Date().toISOString() };
        await route.fulfill({ json: { data }, headers });
        if (heldInitialRead) deliveredInitialReads += 1;
    });
    try {
        await page.goto('/student/verification');
        await expect.poll(() => heldInitialReads).toBeGreaterThan(0);
        await page.getByRole('checkbox', { name: 'Consent notice' }).check();
        await page.getByRole('button', { name: 'Send verification code' }).click();
        const newGrantWithdrawal = page.getByRole('button', { name: 'Withdraw verification consent' });
        await expect(newGrantWithdrawal).toHaveCount(1);
        await newGrantWithdrawal.click();
        await expect(page.getByText('Withdrawn', { exact: true })).toBeVisible();
        await expect(page.getByRole('button', { name: 'Load more consents' })).toBeVisible();
        const expectedHeldInitialReads = heldInitialReads;
        const staleInitialDelivery = page.waitForResponse((response) => (
            response.url() === `${apiOrigin}/api/verification/consents`
            && response.headers()['x-awoof-fixture-initial-consent-read'] === 'true'
        ));
        release();
        const staleResponse = await staleInitialDelivery;
        await staleResponse.finished();
        await expect.poll(() => deliveredInitialReads).toBe(expectedHeldInitialReads);
        await page.evaluate(() => new Promise<void>((resolve) => requestAnimationFrame(() => requestAnimationFrame(() => resolve()))));
        expect(deliveredInitialReads).toBe(expectedHeldInitialReads);
...
for (const eligible of [true, false]) {
    test(`withdraws persisted consent after reload with eligibility=${eligible}`, async ({ page }) => {
        ...
        await page.route(`${apiOrigin}/api/verification/**`, async (route) => {
            ...
            if (path.endsWith('/microsoft/identities')) {
                expect(route.request().method()).toBe('GET'); data = { items: [], nextCursor: null };
            } else if (path.endsWith('/status')) data = { email: 'student@approved.test', universityId: null, eligibility: { eligible: eligible && !withdrawn }, notices: { verification: { version: 'v1', text: 'Consent notice' } } };
...
test('replacement clears private verification inputs and never sends an old grant under the new account', async ({ page, context }) => {
    ...
    await page.route(`${apiOrigin}/api/verification/**`, async (route) => {
        ...
        if (path.endsWith('/microsoft/identities')) {
            expect(route.request().method()).toBe('GET'); data = { items: [], nextCursor: null };
        } else if (path.endsWith('/status')) data = { emailDomainApproved: true, email: second ? 'new@approved.test' : 'private-old@approved.test', universityId: 'school', eligibility: { eligible: false }, notices: { verification: { version: 'v1', text: 'Consent notice' } } };
    ...
    try {
        ...
        // The verification page may deliberately render the current identity
        // in more than one semantic section. Require every rendered exact
        // copy to be visible without coupling the invariant to that count.
        const replacementEmail = page.getByText('new@approved.test', { exact: true });
        await expect.poll(() => replacementEmail.count()).toBeGreaterThan(0);
        for (const copy of await replacementEmail.all()) await expect(copy).toBeVisible();
        ...
        release(); await settled;
        await page.evaluate(() => new Promise<void>((resolve) => requestAnimationFrame(() => requestAnimationFrame(() => resolve()))));
        await expect(page.getByRole('checkbox', { name: 'Consent notice' })).not.toBeChecked();
