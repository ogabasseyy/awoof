...
test('legacy and private identity files require a current owner or admin, while orphaned media is denied', async () => {
    ...
    async function request(directory: string, userId?: string, suffix = '.pdf') {
        let status = 200; let sent = false;
        const headers: Record<string, string> = {};
        const req = { path: `/${directory}/b4e5c64e-c7ab-4f0d-8c12-0d8eeaf269dc${suffix}`, method: 'GET', headers: { authorization: userId ? `Bearer ${jwtService.generateAccessToken({ userId, role: 'vendor', email: 'synthetic@example.invalid' })}` : undefined } } as Request;
    ...
    try {
        for (const directory of ['vendors', 'private-vendors']) {
            ...
            assert.equal((await request(directory, undefined, '')).status, 401);
        ...
        assert.equal((await request('vendors', undefined, '')).status, 404);
        publicMedia = true; assert.equal((await request('vendors', undefined, '')).sent, true); assert.equal((await request('vendors')).sent, true);
